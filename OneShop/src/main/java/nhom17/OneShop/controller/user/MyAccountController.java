...
public class MyAccountController {
    ...
    @PostMapping("/my-account/delete-address/{id}")
    public String deleteAddress(@PathVariable("id") Integer addressId, RedirectAttributes redirectAttributes) { // Đổi tên ra -> redirectAttributes cho rõ nghĩa
        User currentUser = getCurrentUser();
        Optional<Address> optionalAddress = diaChiRepository.findById(addressId);
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("error", "Không thể xóa địa chỉ này vì đang được sử dụng trong một hoặc nhiều đơn hàng.");

        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("error", "Đã xảy ra lỗi không mong muốn khi xóa địa chỉ.");
        }
    ...
    @GetMapping("/order-details/{id}")
    public String orderDetails(@PathVariable("id") Long id, Model model, RedirectAttributes redirectAttributes) { // Thêm RedirectAttributes
        try {
            Order order = orderService.findOrderByIdForCurrentUser(id);
            model.addAttribute("order", order);
            return "user/account/order-details";
        } catch (Exception e) {
            // Thông báo lỗi nếu không tìm thấy đơn hàng hoặc không có quyền xem
            redirectAttributes.addFlashAttribute("error", "Không tìm thấy đơn hàng hoặc bạn không có quyền xem chi tiết đơn hàng này.");
            return "redirect:/my-orders";
        }
    }
    ...
    public String cancelOrder(@PathVariable("id") Long orderId, RedirectAttributes redirectAttributes) {
        try {
            ...
            redirectAttributes.addFlashAttribute("error", "Đã xảy ra lỗi không mong muốn khi hủy đơn hàng.");
        }
        return "redirect:/my-orders";
    }
}